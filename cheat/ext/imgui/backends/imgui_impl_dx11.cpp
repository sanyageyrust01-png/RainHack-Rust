...
// Pre-compiled shader bytecode (no d3dcompiler dependency)
#include "../../../shaders/vs_blob.h"
#include "../../../shaders/ps_blob.h"
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    {
        if (bd->pd3dDevice->CreateVertexShader(g_vsBlob, sizeof(g_vsBlob), nullptr, &bd->pVertexShader) != S_OK)
            return false;
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, g_vsBlob, sizeof(g_vsBlob), &bd->pInputLayout) != S_OK)
            return false;
    ...
    {
        if (bd->pd3dDevice->CreatePixelShader(g_psBlob, sizeof(g_psBlob), nullptr, &bd->pPixelShader) != S_OK)
            return false;
    }
